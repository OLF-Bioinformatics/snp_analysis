...
if (scalar(@ARGV) != 7)
{
   print "Usage: perl snpTableMaker.pl <ref.fasta> <vcfFolder> <minQual> <AC1Report.txt> <section4.txt> <fastaOutFolder> <fastaTable.tsv>\n";
   exit;
}
...
foreach my $handle (@vcfFilesFH)
{
    ...
    while (my $line = <$handle>)
    {
        ...
        my ($CHROM, $POS, $ALT, $QUAL, $INFO) = @fields[0,1,4,5,7];
        my $AC = (split(/;/, $INFO))[0];
        ...
        #AC=1
        push (@{ $AC1{$sample}{$CHROM} }, $POS) if ($AC eq 'AC=1' && $QUAL > $minQual);

        #AC=2
        if ($AC eq 'AC=2' && $QUAL > $minQual)
        {
            push (@{ $AC2{$sample}{$CHROM} }, $POS);
            push (@{ $allAC2{$CHROM} }, $POS);
        }
